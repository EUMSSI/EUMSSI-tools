...
def main():
  for lang in ['en']:
    col.ensure_index([('source_meta.original.entities.user_mentions.screen_name', pymongo.ASCENDING)])
    ...
    limit = {'$limit' : 100000}
    unwind = {'$unwind' : "$source_meta.original.entities.user_mentions" }
    group = {'$group' : {
        '_id' : {'user':'$source_meta.original.user.screen_name', 'mention' : '$source_meta.original.entities.user_mentions.screen_name'},
        'weight' : {'$sum' : 1}
        }}
